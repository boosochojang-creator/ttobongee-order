...
export async function POST(req: NextRequest) {
  try {
    ...
    if ((req.headers.get('content-type') || '').includes('application/json')) {
      ...
      const { data: last } = await admin.from('music_tracks').select('sort_order').eq('store_id', 'baegun').order('sort_order', { ascending: false }).limit(1)
      const nextOrder = (last?.[0]?.sort_order || 0) + 1
      const { error } = await admin.from('music_tracks').insert({ store_id: 'baegun', title: t, url, is_active: true, sort_order: nextOrder })
    ...
    const { error: insErr } = await admin.from('music_tracks').insert({ store_id: 'baegun', title, url: publicUrl, is_active: true, sort_order: nextOrder })
